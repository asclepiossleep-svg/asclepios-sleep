...
export interface TagEffect {
  // Optional: applyTagEffect operates on the `current` TagScore's own tag —
  // it never reads effect.tag. Kept here (rather than removed) because some
  // call sites carry it for logging/traceability; never required.
  tag?: Tag;
