...
async function windowMetrics(userId: string, from: Date, to: Date): Promise<WindowMetrics> {
  ...
  const avgSleepRating = checkins.reduce((a, c) => a + c.sleepRating, 0) / checkins.length;
  const avgNightWakingCount = checkins.reduce((a, c) => a + nightWakingScore(c.nightWakingCount), 0) / checkins.length;
  const avgMorningEnergyScore = checkins.reduce((a, c) => a + energyScore(c.morningEnergy), 0) / checkins.length;
...
export async function run7DayReview(userId: string, decisionVersion = "v1") {
  ...
  const routineDone = routineLogsRecent.filter((l) => l.status === "DONE").length;
  const routineTotal = routineLogsRecent.length || 7;
  const routineCompletionRatio = routineTotal === 0 ? 0 : routineDone / routineTotal;

  const productDoneCount = productLogsRecent.filter((l) => l.status === "DONE").length;
