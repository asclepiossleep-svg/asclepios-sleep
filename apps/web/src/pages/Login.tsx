...
export default function Login() {
  ...
  async function registerWithPassword() {
    ...
    if (password !== confirmPassword) return setError(t("login.passwordMismatch"));
    try {
      const res = await api.post<{ token: string; user: any }>("/auth/password/register", {
        email,
        password,
        name,
        locale,
      ...
      setError(e.message === "account_already_exists" ? t("login.accountExists") : e.message);
    }
  }
  ...
  async function loginWithPassword() {
    setError(null);
    try {
      const res = await api.post<{ token: string; user: any }>("/auth/password/login", { email, password });
      setToken(res.token, res.user);
      navigate(landingRoute(res.user));
    } catch (e: any) {
      setError(e.message === "invalid_credentials" ? t("login.invalidCredentials") : e.message);
    }
  }
  ...
  return (
    <div className="login-page">
      ...
      <div className="login-bottomcontent">
        <div className="card login-card">
          ...
          {authMethod === "password" ? (
            <>
              ...
              <label className="login-field-label" htmlFor="pwEmail">
                {t("login.emailLabel")}
              </label>
              <input
                id="pwEmail"
              ...
              <input
                ...
                className="login-input"
                style={{ marginBottom: mode === "register" ? "0.75rem" : 0 }}
              />
              {mode === "register" && (
                <>
              ...
              <button
                className="primary"
                onClick={mode === "register" ? registerWithPassword : loginWithPassword}
                style={{ marginTop: "0.9rem", width: "100%" }}
                disabled={!email || !password}
              >
                {mode === "register" ? t("login.registerWithPassword") : t("login.loginWithPassword")}
              </button>
              <button type="button" onClick={() => setAuthMethod("code")} className="login-demo-toggle" style={{ marginTop: "0.6rem" }}>
            ...
            <>
              ...
              <button type="button" onClick={() => setAuthMethod("password")} className="login-demo-toggle" style={{ marginTop: "0.6rem" }}>
        ...
        {demoAccessAllowed && demoAccounts.length > 0 && (
          <>
            {!showDemo ? (
