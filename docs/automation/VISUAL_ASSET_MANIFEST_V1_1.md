...
| `approved-asset-hashes` | the sha256 recomputed from disk matches the value recorded in the manifest, and the file's own bytes carry a valid signature for its extension (PNG/JPEG/GIF magic bytes, WebP RIFF/WEBP header, or SVG `<svg`/`<?xml`) — a matching hash is not enough if the file itself is a corrupt or placeholder blob, not real image content |
